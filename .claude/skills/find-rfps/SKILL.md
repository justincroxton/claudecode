...
- **#channel** — Slack channel to post to. Default: **#propellantmediasales** (`C56P28H4J`).
...
Unless `--no-slack`, post one message to the target channel (default
**#propellantmediasales** / `C56P28H4J`) using `slack_send_message`. Format:
...
if a vertical has more, add `…and <x> more in the CSV`. Confirm the channel before
posting only if the user passed a channel that doesn't resolve.
